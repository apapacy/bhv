...
$result = pg_query($db,
"select z.kod,z.parent,c.name as cennic_name, c.izs, c.nv, cex, nop, t.name as tipop_name, raz, s , platmin, platkop, nzpcmin, nzpckop, nzpzmin, nzpzkop" 
...
while ($result and $row = pg_fetch_array($result)) {
    ...
    . '</td><td class="s">' . $row['s']
    . '</td><td class="platmin N">' . str_replace('.', ',', $row['platmin'])
    . '</td><td class="platkop N">' . str_replace('.', ',', $row['platkop'])
