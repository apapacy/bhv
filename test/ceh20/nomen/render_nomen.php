...
for ($i=1; $i<4; $i++){
if ($i==1) $nomen_co_title = 'Поставщик';
else if ($i==2) $nomen_co_title = 'Производство';
else if ($i==3) $nomen_co_title = 'Получатель';
...
echo  "<table  border=1 cellpadding=2 cellspacing=0><tr><th>$nomen_co_title</th><th>Коэффициент</th><th>Размер партии</th><th colspan>Действие</th><td onclick=\'f_insert_nomen_co($kod, $i)\'class=\'nomen_button\' >Добавить</td></tr>";
if ($result) 
	while ($row = pg_fetch_array($result)){
		echo '<tr><td>' . co_full_path($db, $row['co']) . "</td><td>{$row['k']}</td><td>{$row['part']}</td><td onclick=\'f_save_nomen_co({$row['kod']},{$row['type']},{$row['co']}) \'class=\'nomen_button\' >Изменить</td><td  onclick=\'f_delete_nomen_co({$row['kod']},{$row['type']},{$row['co']})\' class=\'nomen_button\'>Удалить</td></tr>";
}
echo '</table><br />';
