...
if ($kod=="new")
	$result = pg_query("insert into zadan (kod,parent,cex,nop,tipop,raz,platmin) values((select max(kod) from zadan)+1,$parent,'$cex',$nop,$tipop,$raz,$platmin)");
else
	$result = pg_query("update zadan set cex='$cex',nop=$nop,tipop=$tipop,raz=$raz,platmin=$platmin,platkop=0 where kod=$kod");
